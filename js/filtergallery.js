jQuery(document).ready(function() {
	...
	// tabs
	// --------------------------------------------------
	jQuery('.de_tab').find('.de_tab_content summary').hide();
	jQuery('.de_tab').find('.de_tab_content summary:first').show();
	
	jQuery('.de_nav li').click(function() {
		setTimeout(() =>{
			$(".de_tab_content")[0]?.scrollIntoView(true);
			window.scrollBy(0,-160);
		}, 400);
		jQuery(this).parent().find('li span').removeClass("active");
		jQuery(this).find('span').addClass("active");
		jQuery(this).parent().parent().find('.de_tab_content summary').hide();
	
		var indexer = jQuery(this).index(); //gets the current index of (this) which is #nav li
		jQuery(this).parent().parent().find('.de_tab_content summary:eq(' + indexer + ')').fadeIn(); //uses whatever index the link has to open the corresponding box 
	});
